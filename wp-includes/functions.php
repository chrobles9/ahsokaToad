...
/******************************************************************
 * Removes version number from source code meta data
 *****************************************************************/
function metaRemoveVersion() {
  return '';
 } add_filter('the_generator', 'metaRemoveVersion');

 /****************************************************************
	* Removes version data from being appended to CSS and JS filenames
  ******************************************************************/
	function cssJsRemoveVersion( $src ) {
    if( strpos( $src, '?ver=' ) )
        $src = remove_query_arg( 'ver', $src );
    return $src;
}
add_filter( 'style_loader_src', 'cssJsRemoveVersion', 9999 );
add_filter( 'script_loader_src', 'cssJsRemoveVersion', 9999 );